...
# Implementation of nodes to place inside the KDTree, a binary tree where every LEAF node is a multi-dimensional point # per: https://en.wikipedia.org/wiki/K-d_tree # TODO: hold indices instead of actual data (for references)
class KDNode():
    # __init__(): constructor for KDNode, takes in point to store as data
    def __init__(self, point_index, left, right, count, indices, dataset):
        self.point = np.reshape(dataset[point_index], (dataset.shape[1], ))
        self.left = left
        self.right = right
...
class KDTree:
    ...
    def __create_tree(self, current_indices, depth):
        ...
        if len(current_indices) == 1:
            return KDNode(point_index = current_indices[0], left = None, right = None, 
                    count = 1, indices = 0, dataset = self.dataset) # no duplicates

        # rotate dimensions
        dimension = depth % self.dimensions

        # get median
        median_index = self.__get_median(dimension, current_indices)

        # split dataset into less than and greater than median along dimension
        l_indices, ge_indices, duplicates = self.__split_dataset(dimension, median_index, current_indices, depth)

        # create node
        node = KDNode(median_index, self.__create_tree(l_indices, depth + 1), self.__create_tree(ge_indices , depth + 1), duplicates, current_indices, self.dataset)
        return node
    ...
    def __split_dataset(self, dimension, median_index, indices, depth):
        data = self.dataset[indices]

        # reshape for broadcasting
        median = np.reshape(self.dataset[median_index], (self.dimensions, ))
    ...
    def __get_median(self, dimension, indices): # get the number to select from the data
        data = self.dataset[indices]
        ...
        # return index of approximate median
